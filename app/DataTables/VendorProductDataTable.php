...
class VendorProductDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function($query){
            $editBtn = "<a href='".route('vendor.products.edit', ['product' => $query])."' class='btn btn-sm btn-primary'><i class='fa fa-edit'></i></a>";
            $delBtn = "<a href='".route('vendor.products.destroy', ['product' => $query])."' class='btn btn-sm btn-danger delete-btn ml-2'><i class='fa fa-trash'></i></a>";
            $moreBtn = ' <div class="dropdown dropleft ml-2 d-inline">
            <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              <i class="fas fa-cog"></i>
            </button>
            <div class="dropdown-menu">
              <a class="dropdown-item has-icon" href="'.route('vendor.products.images.index', ['product' => $query]).'"><i class="far fa-images"></i> Image Gallery</a>
              <a class="dropdown-item has-icon" href="'.route('vendor.products.variants.index', ['product' => $query]).'"><i class="fas fa-tags"></i> Variants</a>
           </div>
          </div>';
            return $editBtn.$delBtn.$moreBtn;
        })
        ->addColumn('approve', function($query){
            return "<select class='form-control is-approved' data-id='".$query->id."' >
        ...
        ->rawColumns(['action', 'thumbnail', 'status', 'type', 'approve']);
    }
    ...
    public function query(Product $model): QueryBuilder
    {
        return $model->newQuery()->where('vendor_id', '<>', auth()->user()->vendor->id)
            ->where('is_approved', 1)->orderBy('created_at');
    }
